...
def run(window_size, window_title, renderer: GCraftRenderer):
    ...
    def _render_fnc():
        ...
        try:
            ...
            renderer.on_render()
            glutSwapBuffers()
            renderer.input_state.clear_mouse_movement()
            ...
            if _frames_count >= _loop_stats_counter:
                ...
                renderer.on_fps(fps, _max_frame_time, _avg_frame_time, _min_frame_time)
    ...
    def _on_mouse_move(x, y):
        ...
        if _just_warped:
            _just_warped = False
            _last_mx = renderer.mouse_warp[0]
            _last_my = renderer.mouse_warp[1]
            return
        ...
        renderer.input_state.update_state(event)
        renderer.on_input(event)

        _last_mx = x
        _last_my = y

        if renderer.mouse_warp is not None:
            _just_warped = True
            glutWarpPointer(renderer.mouse_warp[0], renderer.mouse_warp[1])
    ...
    glutReshapeFunc(renderer.on_reshape)
